"""
NekoCafé Reservation Service — 桌位预约与排队管理
"""
import json
import logging
import os
...
from .routes import availability, matching, queue, reservation
...
def setup_telemetry():
    ...
    resource = Resource.create({SERVICE_NAME: "reservation-service"})
...
async def trace_id_middleware(request: Request, call_next):
    ...
    REQUEST_COUNT.labels(method=method, endpoint=endpoint, status=status, service="reservation-service").inc()
    REQUEST_DURATION.labels(method=method, endpoint=endpoint, service="reservation-service").observe(duration)
