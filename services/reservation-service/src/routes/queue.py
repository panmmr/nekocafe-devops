...
def join_queue(storeId: int):
    ...
    try:
        ...
        conn.commit()
        row = conn.execute("SELECT * FROM queue_tickets WHERE id=?", (cur.lastrowid,)).fetchone()
        return QueueTicket(
            ticketId=row["id"],
...
def get_queue_progress(ticketId: int):
    conn = get_db()
    try:
        row = conn.execute("SELECT * FROM queue_tickets WHERE id=?", (ticketId,)).fetchone()
        if not row:
        ...
        conn.execute("UPDATE queue_tickets SET ahead_count=? WHERE id=?", (ahead, ticketId))
        conn.commit()
        return {
            "ticketId": row["id"],
            "queueNumber": row["queue_number"],
...
def cancel_queue(ticketId: int):
    conn = get_db()
    try:
        conn.execute("UPDATE queue_tickets SET status='EXPIRED' WHERE id=?", (ticketId,))
        conn.commit()
    finally:
        conn.close()
