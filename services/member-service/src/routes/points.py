...
def get_my_points(user: dict = Depends(get_current_user)):
    conn = get_db()
    try:
        row = conn.execute("SELECT points FROM members WHERE id=?", (user["memberId"],)).fetchone()
        txs = conn.execute(
            "SELECT amount, description, created_at FROM points_transactions WHERE member_id=? ORDER BY created_at DESC LIMIT 10",
            (user["memberId"],)
        ).fetchall()
        recent = [{"amount": t["amount"], "description": t["description"], "createdAt": t["created_at"]} for t in txs]
        return PointsInfo(balance=row["points"] if row else 0, recentTransactions=recent)
    finally:
        conn.close()
...
def exchange_points(body: ExchangeRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        conn.execute("UPDATE members SET points=points-100 WHERE id=?", (user["memberId"],))
        conn.execute(
