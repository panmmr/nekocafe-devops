...
def export_my_data(user: dict = Depends(get_current_user)):
    conn = get_db()
    try:
        member = conn.execute("SELECT * FROM members WHERE id=?", (user["memberId"],)).fetchone()
        cats = conn.execute("SELECT * FROM user_cats WHERE member_id=?", (user["memberId"],)).fetchall()
        data = {
            "memberId": member["id"],
            "phone": member["phone"],
        ...
        return {"message": "数据导出请求已接受", "estimatedReadyHours": 24, "data": data}
    finally:
        conn.close()
